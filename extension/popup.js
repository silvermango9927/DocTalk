document.addEventListener('DOMContentLoaded', async () => {
  ...
  signInBtn.addEventListener('click', async () => {
    ...
    if (token) {
      accessToken = token;
      showSignedInState();
      setStatus('success', 'Signed in successfully! Open a Google Doc and click "Extract Document".');
    } else {
  ...
  extractBtn.addEventListener('click', async () => {
    ...
    extractBtn.innerHTML = '<div class="loading"><span class="spinner"></span>Extracting...</div>';
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setStatus('error', `API error: ${response.status} ${response.statusText}`);
        }
        extractBtn.disabled = false;
        extractBtn.textContent = 'Extract Document';
        return;
      ...
      if (extractedText.trim().length === 0) {
        ...
        setStatus('success', 'Content extracted! You can now start a voice conversation.');
  ...
  startVoiceBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (started) {
        ...
        setStatus('success', 'Voice session started! Speak to interact with the agents.');
  ...
  function handleVoiceStatusChange(status) {
    ...
    voiceIndicator.classList.remove('listening', 'speaking', 'processing', 'error');
